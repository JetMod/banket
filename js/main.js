...
const ShenApp = {
  ...
  initGalleryLightbox() {
    ...
    // Поделиться и Скачать (простой лайтбокс: главная, меню, отзывы)
    const shareBtn = lightbox.querySelector('#shareBtn');
    const downloadBtn = lightbox.querySelector('#downloadBtn');

    const showSimpleNotification = (message) => {
      const notification = document.createElement('div');
      notification.className = 'gallery-notification';
      notification.textContent = message;
      notification.style.cssText = 'position:fixed;bottom:30px;left:50%;transform:translateX(-50%);background:rgba(0,0,0,0.9);color:white;padding:16px 32px;border-radius:50px;font-size:14px;z-index:10001;animation:fadeInUp 0.3s ease';
      ...
      shareBtn.addEventListener('click', async (e) => {
        ...
        try {
          if (navigator.share) {
            ...
            showSimpleNotification('Ссылка скопирована в буфер обмена!');
          }
        } catch (err) {
    ...
    if (downloadBtn) {
      downloadBtn.addEventListener('click', (e) => {
        ...
        link.href = url;
        link.download = `shen-gallery-${currentIndex + 1}.jpg`;
        link.click();
        showSimpleNotification('Загрузка началась...');
      });
    }
  }
};
